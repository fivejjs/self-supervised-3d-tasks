...
class SegmentationGenerator3D(DataGeneratorBase):
    def __init__(
        ...
        super(SegmentationGenerator3D, self).__init__(
        ...
        if np.random.rand() < 0.5:
            ...
            if axis_choice == 0:
                ...
                processed_image = ndimage.rotate(
                    processed_image, xy_angle, axes=(0, 1), reshape=False, order=1
                )
                processed_mask = ndimage.rotate(
                    processed_mask, xy_angle, axes=(0, 1), reshape=False, order=0
                )
            elif axis_choice == 1:
                yz_angle = np.random.uniform(0, 360)
                processed_image = ndimage.rotate(
                    processed_image, yz_angle, axes=(1, 2), reshape=False, order=1
                )
                processed_mask = ndimage.rotate(
                    processed_mask, yz_angle, axes=(1, 2), reshape=False, order=0
                )
            else:
                xz_angle = np.random.uniform(0, 360)
                processed_image = ndimage.rotate(
                    processed_image, xz_angle, axes=(0, 2), reshape=False, order=1
                )
                processed_mask = ndimage.rotate(
                    processed_mask, xz_angle, axes=(0, 2), reshape=False, order=0
                )
    ...
    def data_generation(self, list_files_temp):
        ...
        for file_name in list_files_temp:
            ...
            path_label = path_label.with_name(
                path_label.stem + self.label_stem
            ).with_suffix(path_label.suffix)

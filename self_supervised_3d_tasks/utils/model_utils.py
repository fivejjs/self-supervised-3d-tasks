...
from self_supervised_3d_tasks.models.fully_connected import (
    fully_connected_big,
    simple_multiclass,
)
...
def get_prediction_model(
    name, in_shape, include_top, algorithm_instance, num_classes, kwargs
):
    if name == "big_fully":
        ...
        assert (
            algorithm_instance is not None
        ), "no algorithm instance for 2d skip connections found"
        assert (
            algorithm_instance.layer_data is not None
        ), "no layer data for 2d skip connections found"
        ...
        inputs_skip = [
            Input(x.shape[1:]) for x in reversed(algorithm_instance.layer_data[0])
        ]
        ...
        assert (
            algorithm_instance is not None
        ), "no algorithm instance for 3d skip connections found"
        assert (
            algorithm_instance.layer_data is not None
        ), "no layer data for 3d skip connections found"
        ...
        last_out = Reshape((n_patches,) + model_up.layers[-1].output_shape[1:])(
            last_out
        )
...
def apply_prediction_model_to_encoder(
    ...
    **kwargs
    ...
    sub_model = apply_prediction_model(
        (units,),
        n_prediction_layers,
        dim_prediction_layers,
...
def make_finetuning_encoder(
    input_shape, enc_model, f_encoder_model, t_pooling, **kwargs
):
    new_enc_model, layer_data = f_encoder_model(input_shape, **kwargs)
    ...
    if layer_data:
        ...
        model_skips = Model(
            inputs=new_enc_model.inputs,
            outputs=[new_enc_model.layers[-1].output, *reversed(layer_data[0])],
        )
...
def make_finetuning_encoder_2d(input_shape, enc_model, **kwargs):
    return make_finetuning_encoder(
        input_shape, enc_model, apply_encoder_model, Pooling2D, **kwargs
    )


def make_finetuning_encoder_3d(input_shape, enc_model, **kwargs):
    return make_finetuning_encoder(
        input_shape, enc_model, apply_encoder_model_3d, Pooling3D, **kwargs
    )

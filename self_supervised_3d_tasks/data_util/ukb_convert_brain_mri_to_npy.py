...
def convert_mri_unlabeled():
    ...
    t1_zip_files = sorted(
        glob.glob(
            source_path + "T1_structural_brain_mri/archive/**/" + "*.zip",
            recursive=True,
        )
    )
...
def convert_mri_masks(type="fast"):
    ...
    filename_list = sorted(
        glob.glob(destination_path + "T1/**/" + "*.npy", recursive=True)
    )
    ...
    for filename in filename_list:
        ...
        if type == "fast":
            mask = nib.load(
                source_path + str(id) + "_20252_2_0/T1/T1_fast/T1_brain_seg.nii.gz"
            ).get_data()
            np.save(
                os.path.join(destination_path, "fast_masks", str(id) + ".npy"), mask
            )
        else:
            mask = nib.load(
                source_path
                + str(id)
                + "_20252_2_0/T1/T1_first/T1_first_all_fast_firstseg.nii.gz"
            ).get_data()
            np.save(
                os.path.join(destination_path, "first_masks", str(id) + ".npy"), mask
            )

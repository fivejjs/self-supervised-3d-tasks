...
def neighbour_smoothing(values, epochs, neighbour_count=2):
    ...
    for index in range(int(len(values) / (neighbour_count))):
        smoothed_values.append(
            np.mean(
                values[
                    index * (neighbour_count) : index * (neighbour_count)
                    + neighbour_count
                ]
            )
        )
...
def draw_epoch_plot(
    ...
    for index, path in enumerate(paths):
        ...
        try:
            ...
            with open(config, mode="r") as file:
                args = json.load(file)
            epochs, values = get_metric_over_epochs(
                args, path, metric, nth_epoch=nth_epoch
            )
    ...
    plt.legend(
        loc=2,
        bbox_to_anchor=(0.65, 0.05 + len(data_names) * 0.08),
        fontsize=30,
        borderaxespad=0.0,
    )
    ...
    plt.yticks(
        np.arange(round(min_value, 1), round(max_value, 1) + 0.04, 0.02), fontsize=24
    )
    plt.xticks(fontsize=24)

    plt.grid()
    plt.savefig(
        "plots/" + "_".join(["epochs", prefix, *data_names]) + ".pdf",
        bbox_inches="tight",
    )
    plt.show()


def draw_train_split_plot(
    paths, data_names, metric, skips=[], prefix="2d", metric_name=None
):
    plt.figure(figsize=(15, 10))
    min_value = inf
    max_value = -inf
    ...
    plt.yticks(
        np.arange(round(min_value, 1), round(max_value, 1) + 0.1, 0.05), fontsize=24
    )
    plt.xticks(splits, fontsize=24)

    plt.grid()
    plt.savefig(
        "plots/" + "_".join(["trainsplit", prefix, *data_names]) + ".pdf",
        bbox_inches="tight",
    )
    plt.show()
...
def draw_brats_plot():
    ...
    plt.legend(
        loc=2,
        bbox_to_anchor=(0.65, 0.05 + len(names) * 0.08),
        fontsize=30,
        borderaxespad=0.0,
    )
    ...
    plt.grid()
    plt.savefig(
        "plots/" + "_".join(["trainsplit", "brats"]) + ".pdf", bbox_inches="tight"
    )
    plt.show()
...
if __name__ == "__main__":
    ...
    cpc_label = "cpc"
    cpc_2d_path = (
        "~/workspace/self-supervised-3d-tasks/cpc_kaggle_retina/weights-250_test_4/"
    )
    cpc_3d_path = (
        "~/workspace/self-supervised-3d-tasks/cpc_pancreas3d_18/weights-400_test_3"
    )
    ...
    exemplar_3d_path = (
        "~/workspace/self-supervised-3d-tasks/exemplar_pancreas3d_42/weights-300_test_3"
    )
    ...
    cpc_new_3d_path = (
        "~/workspace/self-supervised-3d-tasks/cpc_pancreas2d_1/weights-250_test"
    )
    ...
    draw_train_split_plot(
        combined_3d_path,
        combined_labels,
        split_3d_metric,
        skips=[],
        prefix="3d",
        metric_name=split_3d_metric_name,
    )
    ...
    draw_epoch_plot(
        ...
        neighbour_count=10,
        prefix="3d",
        metric_name=epoch_3d_metric_name,
    )
    ...
    for index in range(len(combined_3d_path) - 1):
        ...
        draw_train_split_plot(
            ...
            split_2d_metric,
            skips=[1],
            metric_name=split_2d_metric_name,
        )
